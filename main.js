...
Таймер должен получать время обратного отсчёта в конструкторе.
Должен иметь кнопки старт(запускает) и стоп(ставит паузу).
Должен иметь в конструторе параметр для автоматического запуска(true/false).
Длина полоски уменьшается с движением таймера.*/

const container = document.querySelector('.container');

class Timer {
    constructor(period, isAuto = false) {
        this.period = period * 60;
        this.currentTime = period * 60;
        this.isAuto = isAuto;
    ...
    status = (t) => {
        ...
        const perc = (1 - (t.period - t.currentTime) / t.period) * 100;
